...
def get_scannet_superpoints(mesh_path, superpoint_path=None):
    # load mesh vertices and colors

    superpoint = np.loadtxt(superpoint_path).astype(np.int64)
    # import pdb; pdb.set_trace()    
    return superpoint

def get_points_from_ply(ply_path, out_path):
    ply = plyfile.PlyData.read(ply_path)
    xs = np.array(ply["vertex"].data['x'])[:, None]
    ys = np.array(ply["vertex"].data['y'])[:, None]
    zs = np.array(ply["vertex"].data['z'])[:, None]
    points = np.concatenate((xs, ys, zs), axis=-1)
    np.savetxt(join(dirname(out_path), 'points.pts'), points)
...
def torch_get_similar_confidence_matrix_multiview_3(seg_neighbors,
                                        seg_ids,
                                        seg_seen,
                                        points_label,
                                        similar_metric,
    ...
    gpu_label_seen = torch.tensor(seg_label_seen0, device=device, dtype=torch.float32)  # (s,m)

    similar_sum = torch.zeros([seg_num, seg_num], device=device, dtype=torch.float32)  # (s,s)
    similar_sum_last = torch.zeros([seg_num, seg_num], device=device, dtype=torch.float32)  # (s,s)
    multiview_similar_sum = torch.zeros([seg_num, seg_num], device=device, dtype=torch.float32)  # (s,s)
    confidence_sum = torch.zeros([seg_num, seg_num], device=device, dtype=torch.float32)
    confidence_sum_last = torch.zeros([seg_num, seg_num], device=device, dtype=torch.float32)
    ...
    multiview_label_range = int(torch.max(gpu_points_label) + 1) # 包含0
    multiview_seglabels = torch.zeros([seg_num, view_num, multiview_label_range], device=device, dtype=torch.float32)  # (s,m,lr)
    multiview_confidence_sum = torch.zeros([seg_num, seg_num], device=device, dtype=torch.float32)
    multiview_similar = torch.zeros([seg_num, seg_num], device=device, dtype=torch.float32)
    multiview_confidence = torch.zeros([seg_num, seg_num], device=device, dtype=torch.float32)
    ...
    for m in tqdm(range(view_num)):
        ...
        # unique_labels_segids = [点的mask_label, Superpoint的id]
        unique_labels_segids = unique_labels_segids.type(torch.long)
        unique_counts = unique_counts.type(torch.float32)
        # seglabels, Superpoint的id对应的每个labei值, seglabels[0][0]意思是seg_id为0，且mask_label为0的数量
        seglabels[unique_labels_segids[:, 1], unique_labels_segids[:, 0]] = unique_counts  # (s,lr)
        
        multiview_seglabels[unique_labels_segids[:, 1], m, unique_labels_segids[:, 0]] = unique_counts
        # multiview_nonzero_seglabels = multiview_seglabels[:, m, 1:]
        
        # exclude invalid label
        nonzero_seglabels = seglabels[:, 1:]   # (s,lr-1) 0为背景类，排除
        ...
        for start_id in range(0, seg_num, batch_size):
            ...
            one_view_similar[start_id:start_id+batch_size, all_neighbors_mask] = \
                torch_calcu_all_similar(nonzero_seglabels[start_id:start_id+batch_size], # 当前节点的label 值为数量
                                        neighbors_labels, # 邻居节点的label 值为数量
                                        similar_metric=similar_metric,
                                        thres_trunc=thres_trunc)
        # (s,1) @ (1,s) = (s,s)
    ...
    for m in tqdm(range(1,view_num)):
        # each view
        plabels = gpu_points_label[:, m].to(device)  # (N,)
        plabels_last = gpu_points_label[:, m-1].to(device)  # (N,)
        one_view_similar.fill_(0.)
        one_view_confidence.fill_(0.)

        label_range = int(torch.max(plabels) + 1)
        label_range_last = int(torch.max(plabels_last) + 1)
        label_range = max(label_range, label_range_last)
        if (label_range < 2):
            continue  # there's no valid mask in the view

        # get label distribution of all segs in the view
        seglabels = torch.zeros([seg_num, label_range], device=device, dtype=torch.float32)  # (s,lr)
        seglabels_last = torch.zeros([seg_num, label_range], device=device, dtype=torch.float32)  # (s,lr)

        p_labels_segids = torch.stack([plabels, gpu_seg_ids], dim=1)
        p_labels_segids_last = torch.stack([plabels_last, gpu_seg_ids], dim=1)

        unique_labels_segids, unique_counts = \
            torch.unique(p_labels_segids, return_counts=True, dim=0)
        unique_labels_segids_last, unique_counts_last = \
            torch.unique(p_labels_segids_last, return_counts=True, dim=0)
        
        # unique_labels_segids = [点的mask_label, Superpoint的id]
        unique_labels_segids = unique_labels_segids.type(torch.long)
        unique_counts = unique_counts.type(torch.float32)
        unique_labels_segids_last = unique_labels_segids_last.type(torch.long)
        unique_counts_last = unique_counts_last.type(torch.float32)
        
        # seglabels, Superpoint的id对应的每个labei值, seglabels[0][0]意思是seg_id为0，且mask_label为0的数量
        seglabels[unique_labels_segids[:, 1], unique_labels_segids[:, 0]] = unique_counts  # (s,lr)
        seglabels_last[unique_labels_segids_last[:, 1], unique_labels_segids_last[:, 0]] = unique_counts_last  # (s,lr)
        
        # multiview_seglabels[unique_labels_segids[:, 1], m, unique_labels_segids[:, 0]] = unique_counts
        # multiview_seglabels[unique_labels_segids_last[:, 1], m-1, unique_labels_segids_last[:, 0]] = unique_counts_last
        
        # multiview_nonzero_seglabels = multiview_seglabels[:, m, 1:]
        
        # exclude invalid label
        nonzero_seglabels = seglabels[:, 1:]   # (s,lr-1) 0为背景类，排除
        nonzero_seglabels_last = seglabels_last[:, 1:]   # (s,lr-1) 0为背景类，排除
        ...
        if (similar_metric == "2-norm"):
            nonzero_seglabels = torch.divide(
                nonzero_seglabels, 
                torch.clamp(torch.norm(nonzero_seglabels, dim=-1), 1e-8)[:, None])
            nonzero_seglabels_last = torch.divide(
                nonzero_seglabels_last, 
                torch.clamp(torch.norm(nonzero_seglabels_last, dim=-1), 1e-8)[:, None])
        else:
            raise NotImplementedError

        del unique_counts, unique_labels_segids
        del p_labels_segids, plabels
        del unique_counts_last, unique_labels_segids_last
        del p_labels_segids_last, plabels_last
        ...
        for start_id in range(0, seg_num, batch_size):
            ...
            neighbors_labels = nonzero_seglabels[all_neighbors_mask]
            neighbors_labels_last = nonzero_seglabels_last[all_neighbors_mask]
            
            # one_view_similar[start_id:start_id+batch_size, all_neighbors_mask] = \
            #     torch_calcu_all_similar(nonzero_seglabels[start_id:start_id+batch_size], 
            #                             neighbors_labels_last, 
            #                             similar_metric=similar_metric,
            #                             thres_trunc=thres_trunc)
            current_last_similar = torch_calcu_all_similar(nonzero_seglabels[start_id:start_id+batch_size], 
                                        neighbors_labels_last, 
                                        similar_metric=similar_metric,
                                        thres_trunc=thres_trunc)
            last_current_similar = torch_calcu_all_similar(nonzero_seglabels_last[start_id:start_id+batch_size], 
                                        neighbors_labels, 
                                        similar_metric=similar_metric,
                                        thres_trunc=thres_trunc)
            one_view_similar[start_id:start_id+batch_size, all_neighbors_mask] = (current_last_similar + last_current_similar)/2
            # one_view_similar[start_id:start_id+batch_size, all_neighbors_mask] = current_last_similar
        # (s,1) @ (1,s) = (s,s)
        one_view_confidence = gpu_seg_seen[:,m][:, None] @ gpu_seg_seen[:, m][None, :]
        one_view_confidence_last = gpu_seg_seen[:,m-1][:, None] @ gpu_seg_seen[:, m-1][None, :]
        # one_view_confidence = (one_view_confidence + one_view_confidence_last)/2
        one_view_confidence = (one_view_confidence * one_view_confidence_last)
        del seglabels, nonzero_seglabels
        del seglabels_last, nonzero_seglabels_last

        confidence_sum_last += one_view_confidence
        similar_sum_last += (one_view_similar*one_view_confidence)


    return [similar_sum.cpu().numpy(), [confidence_sum.cpu().numpy(), confidence_sum_last.cpu().numpy()], similar_sum_last.cpu().numpy()]
    # else:
    #     return [similar_sum.cpu().numpy(), confidence_sum.cpu().numpy()]


@torch.inference_mode()
def torch_get_similar_confidence_matrix_multiview_4(seg_neighbors,
                                        seg_ids,
                                        seg_seen,
                                        seg_distance,
                                        points_label,
    ...
    gpu_seg_distance = torch.tensor(seg_distance, device=device, dtype=torch.float32)  # (s,m)
    ...
    one_view_similar = torch.zeros([seg_num, seg_num], device=device, dtype=torch.float32)
    one_view_confidence = torch.zeros([seg_num, seg_num], device=device, dtype=torch.float32)
    one_view_distance = torch.zeros([seg_num, seg_num], device=device, dtype=torch.float32)
    ...
    for m in tqdm(range(view_num)):
        ...
        m_distance = gpu_seg_distance[:,m]
        if not torch.isnan(m_distance).all():
            max_m_distance = torch.max(m_distance[~torch.isnan(m_distance)])
            min_m_distance =  torch.min(m_distance[~torch.isnan(m_distance)])
            distance_weights = 0.9 * (max_m_distance - m_distance) / (max_m_distance - min_m_distance) + 0.1
            distance_weights[torch.isnan(m_distance)] = 0
            one_distance_confidence = distance_weights[:, None] @ distance_weights[None, :]

        else:
            distance_weights = 0
            one_distance_confidence = distance_weights

        del seglabels, nonzero_seglabels
        del m_distance, max_m_distance, min_m_distance, distance_weights

        confidence_sum += one_view_confidence
        similar_sum += (one_view_similar*one_view_confidence)

        # confidence_sum += (one_view_confidence*one_distance_confidence)
        # similar_sum += (one_view_similar*one_view_confidence*one_distance_confidence)
    gpu_seg_distance[gpu_seg_seen<0.1]= float('nan')
    gpu_distance_seen = gpu_seg_distance.clone()

    nan_mask = torch.isnan(gpu_distance_seen)
    distance_seen_1 = gpu_distance_seen.clone()
    distance_seen_2 = gpu_distance_seen.clone()
    distance_seen_1[nan_mask] = float('inf')
    distance_seen_2[nan_mask] = -float('inf')

    distance_seen_ratio_min, min_coord =  torch.min(distance_seen_1,dim=1)
    distance_seen_ratio_max, max_coord=  torch.max(distance_seen_2,dim=1)
    
    d_max = distance_seen_ratio_max.unsqueeze(1).repeat(1, gpu_distance_seen.shape[1])
    d_min = distance_seen_ratio_min.unsqueeze(1).repeat(1, gpu_distance_seen.shape[1])
    
    distance_seen_ratio = (d_max - gpu_distance_seen) / (d_max - d_min)
    distance_seen_ratio[torch.isnan(distance_seen_ratio)] = 0
    distance_seen_ratio *= gpu_seg_seen

    start_key_id = 0
    if key_list_path is not None:
        with open(key_list_path, 'r') as file:
            text = file.read()
        key_list = [int(num) for num in text.split(', ')]
    else:
        print("No key")
        sys.exit(1)  
    ...
    for keyid in key_list:
        ...
        seg_id_frame_weight_max, seg_id_frame_weight_coord = torch.max(distance_seen_ratio[:,start_key_id:keyid],dim=1)
        # seg_id_frame_weight_max=0?
        seg_num_tensor = torch.arange(seg_num).to(device)
        seg_max_coord = torch.stack([seg_num_tensor, seg_id_frame_weight_coord], dim=1)
        seglabels = multiview_seglabels[:,start_key_id:keyid,:][seg_max_coord[:,0],seg_max_coord[:,1],:]
        ...
        for start_id in range(0, seg_num, batch_size):
            ...
            current_last_similar = torch_calcu_all_similar(nonzero_seglabels[start_id:start_id+batch_size], # 当前节点的label 值为数量
                                        neighbors_labels, 
                                        similar_metric=similar_metric,
                                        thres_trunc=thres_trunc)
            ...
            # one_view_similar[start_id:start_id+batch_size, all_neighbors_mask] = (current_last_similar + last_current_similar)/2
            one_view_similar[start_id:start_id+batch_size, all_neighbors_mask] = current_last_similar
            # one_view_similar[start_id:start_id+batch_size, all_neighbors_mask] = last_current_similar
        # (s,1) @ (1,s) = (s,s)
        # one_view_confidence = gpu_seg_seen[:,start_key_id:keyid] @ gpu_seg_seen[:, start_key_id:keyid].T
        one_view_confidence = gpu_seg_seen[seg_max_coord[:,0],seg_max_coord[:,1]][:, None] @ gpu_seg_seen[seg_max_coord[:,0],seg_max_coord[:,1]][None, :]
        # one_view_confidence = (one_view_confidence + one_view_confidence_last)/2
        # one_view_confidence = (one_view_confidence * one_view_confidence_last)
        # one_view_confidence = (one_view_confidence)
        del seglabels, nonzero_seglabels

        # seg_id_frame_flag = (seg_id_frame_weight_max>0.5).int()
        # seg_id_frame_weight_max *= seg_id_frame_flag
        
        # seg_id_frame_weight_max = gpu_seg_seen[seg_max_coord[:,0],seg_max_coord[:,1]]*seg_id_frame_weight_max
        one_view_distance = seg_id_frame_weight_max[:, None] @ seg_id_frame_weight_max[None, :]


            
        confidence_sum_last += one_view_distance
        similar_sum_last += (one_view_similar*one_view_distance)

        # confidence_sum_last += (one_view_confidence*one_view_distance)
        # similar_sum_last += (one_view_similar*one_view_confidence*one_view_distance)
...
@torch.inference_mode()
def torch_get_similar_confidence_matrix_multiview(seg_neighbors,
                                        seg_ids,
                                        seg_seen,
                                        points_label,
                                        similar_metric,
    ...
    multiview_nonzero_seglabels = multiview_seglabels.permute(0,2,1).contiguous()
    if multi_view:
        for l in range(1, multiview_seglabels.shape[2]):
            one_view_similar.fill_(0.)
            one_view_confidence.fill_(0.)
            multiview_nonzero_seglabels_tmp = multiview_nonzero_seglabels[:,l,:].clone() # [s, m]

            if (similar_metric == "2-norm"):
                multiview_nonzero_seglabels_tmp = torch.divide(
                    multiview_nonzero_seglabels_tmp, 
                    torch.clamp(torch.norm(multiview_nonzero_seglabels_tmp, dim=-1), 1e-8)[:, None])
            else:
                raise NotImplementedError
            ...
            for start_id in range(0, seg_num, batch_size):
                ...
                all_neighbors_mask = \
                    torch.sum(gpu_seg_neighbors[start_id:start_id+batch_size], dim=0) > 0
                multiview_neighbors_labels = multiview_nonzero_seglabels_tmp[all_neighbors_mask]
                multiview_similar[start_id:start_id+batch_size, all_neighbors_mask] = \
                    torch_calcu_all_similar(multiview_nonzero_seglabels_tmp[start_id:start_id+batch_size], # 当前节点的label 值为数量
                                            multiview_neighbors_labels, # 邻居节点的label 值为数量
                                            similar_metric=similar_metric,
                                            thres_trunc=thres_trunc)

            one_view_confidence = gpu_label_seen[:,l-1][:, None] @ gpu_label_seen[:,l-1][None, :]
            multiview_confidence_sum += one_view_confidence
            multiview_similar_sum += (multiview_similar * one_view_confidence)

        return [similar_sum.cpu().numpy(), [confidence_sum.cpu().numpy(), multiview_confidence_sum.cpu().numpy()], multiview_similar_sum.cpu().numpy()]
    else:
        return [similar_sum.cpu().numpy(), confidence_sum.cpu().numpy()]
...
def torch_calcu_all_similar(labels1, labels2, similar_metric="2-norm", thres_trunc=0):
    ...
    s1 = labels1.shape[0]
    s2 = labels2.shape[0]
    index1 = torch.tile(torch.arange(s1)[:, np.newaxis], (1, s2))  
    index2 = torch.tile(torch.arange(s2), (s1, 1))  
    if (similar_metric == '2-norm'):
        # (s1,s2,lr-1)->(s1,s2)
    ...
    zero_rows_l1 = torch.all(labels1 == 0, dim=1)
    zero_rows_l2 = torch.all(labels2 == 0, dim=1)

    zero_row_indices_l1 = torch.nonzero(zero_rows_l1).squeeze()
    zero_row_indices_l2 = torch.nonzero(zero_rows_l2).squeeze()
    if zero_row_indices_l1 is not None:
        similar[zero_row_indices_l1] = 0
        # similar[:,zero_row_indices_l1] = 0
   
    if zero_row_indices_l2 is not None:
        # similar[zero_row_indices_l2] = 0
        similar[:,zero_row_indices_l2] = 0
...
@torch.inference_mode()
def torch_calcu_all_similar_multiview(labels1, labels2, similar_metric="2-norm", thres_trunc=0):
    '''
    :param labels0: (s1, m, lr-1), label distribuion of one seg 
    :param labels1: (s2, m, lr-1), label distribution of neighbors of the seg above
    :return similar: (s1,s2) pairwise similar
    '''
    s1 = labels1.shape[0]
    s2 = labels2.shape[0]
    ...
    if (similar_metric == '2-norm'):
        # (s1,s2,lr-1)->(s1,s2)
        dis = torch.norm(labels1[index1].view(s1, s2, -1) - labels2[index2].view(s1, s2, -1), dim=-1)
        def similar_func(x): return (2-x**2)/2.  # equal to cosine similarity
    else:
        raise NotImplementedError
...
def get_scannetpp_mask(color_path, base_dir, scene_id, mask_group_name, color_shape):
    mask_dir = join(base_dir, '2D_masks_%5_afcvpr', scene_id, mask_group_name)
    color_name = basename(color_path)

    color_name = str(int(color_name.split('.')[0].split('_')[-1]))+'.jpg'
    # import pdb; pdb.set_trace()
    mask_name = 'maskraw_' + color_name.replace('jpg', 'png')
    mask_path = join(mask_dir, mask_name)
    mask_raw = cv2.imread(mask_path, -1).astype(np.float32)
...
def multiview_get_similar_confidence_matrix_handle(seg_neighbors,
                                         seg_ids,
                                         seg_seen,
                                         seg_distance,
                                         points_label,
    ...
    for m in tqdm(range(view_num)):
        ...
        if all_similiar:
            for i in range(seg_neighbors.shape[0]):
                ...
                one_view_similar[i, :] = calcu_similar(nonzero_seglabels[i],
                                                        nonzero_seglabels,
                                                        similar_metric=similar_metric,
                                                        thres_trunc=thres_trunc)
                # (nei,) * () = (nei,)
                one_view_confidence[i, :] = seg_seen[:, m]*seg_seen[i, m]            
        else:
            for i in range(seg_neighbors.shape[0]):
                if (seg_neighbors[i].nonzero()[0].size == 0):
                    continue
                for seg_nei in seg_neighbors[i]:
                    neighbors_labels = nonzero_seglabels[seg_nei]
                    one_view_similar[i, seg_nei] = calcu_similar(nonzero_seglabels[i],
                                                                        neighbors_labels,
                                                                        similar_metric=similar_metric,
                                                                        thres_trunc=thres_trunc)
                    # (nei,) * () = (nei,)
                    one_view_confidence[i, seg_nei] = seg_seen[seg_nei, m]*seg_seen[i, m]


        m_distance = seg_distance[:,m]
        m_distance[seg_seen[:,m]==0]=np.nan
        max_values = np.max(nonzero_seglabels, axis=1)
        purity = max_values[:, None] @ max_values[None, :]

        if not np.isnan(m_distance).all():
            # max_m_distance = np.max(m_distance[~np.isnan(m_distance)])
            # min_m_distance =  np.min(m_distance[~np.isnan(m_distance)])
            max_m_distance = np.nanmax(m_distance)
            min_m_distance = np.nanmin(m_distance)
            # import pdb; pdb.set_trace()
            # distance_weights =  (max_m_distance - m_distance) / (max_m_distance - min_m_distance)

            # distance_weights = 0.9 * (max_m_distance - m_distance) / (max_m_distance - min_m_distance) + 0.1
            distance_weights = 0.9 * np.power((max_m_distance - m_distance) / (max_m_distance - min_m_distance),2) + 0.1
            distance_weights[np.isnan(m_distance)] = 0
            one_distance_confidence = distance_weights[:, None] @ distance_weights[None, :]
            one_distance_confidence = one_distance_confidence * purity
            del m_distance, max_m_distance, min_m_distance, distance_weights
        else:
            distance_weights = 0
            one_distance_confidence = distance_weights
            del distance_weights, m_distance


        similar_sum = similar_sum + one_view_similar*one_view_confidence*one_distance_confidence
        confidence_sum += one_view_confidence*one_distance_confidence

        
    return [similar_sum, confidence_sum]

def multiview_multiprocess_get_similar_confidence_matrix(seg_seen, 
                                               seg_neighbors, 
                                               seg_ids, 
                                               seg_distance,
                                               points_label, 
    ...
    for i in range(process_num):
        process_seg_seen = seg_seen[:,i*view_per_process:(i+1)*view_per_process]  # (s,b)
        process_seg_distance = seg_distance[:,i*view_per_process:(i+1)*view_per_process]  # (s,b)
        # (N,b)
        # import pdb; pdb.set_trace()
        process_plabels = points_label[:,i*view_per_process:(i+1)*view_per_process]


        res = pool.apply_async(multiview_get_similar_confidence_matrix_handle,
                               args=(seg_neighbors, seg_ids,
                                     process_seg_seen, process_seg_distance, process_plabels,
                                     similar_metric,
                                     thres_trunc,
                                     all_similiar))


        # res = multiview_get_similar_confidence_matrix_handle(seg_neighbors, seg_ids,
        #                              process_seg_seen, process_seg_distance, process_plabels,
        #                              similar_metric,
        #                              thres_trunc)
        res_list.append(res)
